...
def store_subject(file_path, cur, connect):
    ...
    sql = '''insert into subject_node_table (node_uuid, hash_id, exec)
                         values (?, ?, ?)
            '''
    #ex.execute_values(cur, sql, datalist, page_size=10000)
    cur.executemany(sql, datalist)
...
def store_file(file_path, cur, connect):
    ...
    sql = '''insert into file_node_table (node_uuid, hash_id, path)
                         values (?, ?, ?)
            '''
    #ex.execute_values(cur, sql, datalist, page_size=10000)
    cur.executemany(sql, datalist)
...
def create_node_list(cur, connect):
    ...
    sql = '''insert into node2id (hash_id, node_type, msg, index_id)
                         values (?, ?, ?, ?)
            '''
    #ex.execute_values(cur, sql, node_list_database, page_size=10000)
    cur.executemany(sql, node_list_database)
    connect.commit()
...
def store_event(file_path, cur, connect, reverse, nodeid2msg, subject_uuid2hash, file_uuid2hash, net_uuid2hash):
    ...
    sql = '''insert into event_table (src_node, src_index_id, operation, dst_node, dst_index_id, timestamp_rec)
                         values (?, ?, ?, ?, ?, ?)
            '''
    #ex.execute_values(cur, sql, datalist, page_size=10000)
    cur.executemany(sql, datalist)
